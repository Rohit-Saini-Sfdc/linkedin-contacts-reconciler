...
class GooglePeopleApi {
  ...
  static async searchContacts(token, query) {
    ...
    // Clean search term (remove special chars)
    ...
    try {
      ...
      console.warn("searchContacts error, falling back to full connections scan...", err);
    }

    // Fallback: Scan full connections list with token matching
  ...
  static async searchConnectionsFallback(token, query) {
    ...
    try {
      do {
        const endpoint = `people/me/connections?pageSize=1000&personFields=${personFields}${pageToken ? `&pageToken=${pageToken}` : ''}`;
        const data = await this.request(endpoint, token);
        if (data.connections) {
      ...
      console.warn("Failed fetching full connections list:", e);
    }
    ...
    return connections
      .filter(p => {
        ...
        return queryTokens.every(tok => fullText.includes(tok));
      })
      .slice(0, 20)
